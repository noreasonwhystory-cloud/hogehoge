...
<div class="box">
Nansen/leaderboardが「プロ」と示した層を<b>窓アーティファクト/高頻度MM/塩漬け/alt偏重/運</b>の軸で多角検証し、品質を6段階(エリート{n_elite}・堅実{wq.get('堅実',0)}・ムラあり{wq.get('ムラあり',0)}・履歴薄{wq.get('履歴薄/評価不能',0)}・格付け過大{wq.get('格付け過大',0)}・高頻度MM{wq.get('高頻度MM',0)})に整理。
