...
<br><span style="color:#7fd6a8">※ 方向先読み判定は当初 majors(BTC/ETH/SOL)限定だったが、2026-06-22 にビルダー配備perp(個別株/半導体/プレIPO/商品)も全perpで再走査＋敵対裁定し、24候補全件が MM/セクターβ/薄商いN=1/トレンド保有へ収束＝<b>全perp含めても個人インサイダー未検出で確定</b>（要監視 0x74fc5e 1件のみ）。</span>
</div>
...
最終的に「遅効エッジ ＋ 分散 ＋ majors実現益が黒字(>$5万) ＋ 局所ベースライン検証」を満たした<b class="g">5件</b>を🟠弱い疑惑(監視継続)とした(当初6件→全期間平均では拾えたSOL地合便乗1件を局所基準で除外)。
