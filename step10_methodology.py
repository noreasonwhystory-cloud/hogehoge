...
<li><b>方向的中率（複数地平線）</b>: 各エントリ後に価格が建玉方向へ動いた割合。当初は<b>{C.HIT_HORIZON_H}時間後の一点固定</b>だったが、これだと<u>数日かけて効くポジショニングをコイン投げと誤判定</u>するため、現在は <b>1/4/12/24/48/72h</b> で測りトレンド補正もかける。4hは≈0でも24-72hで市場ドリフトを超える「<b>遅効エッジ</b>」を持つ層が実在した。</li>
</ol>
<div class="eg">例: ETHを18:31に$20万ショート → 22:00から-4%下落。＝下落の3.5時間前に下落方向へ大口 → 「先行」と判定。</div>
<div class="box bad" style="margin-top:8px"><b>遅効エッジの注意:</b> 「建玉後72hに順方向へ動いた」は「儲けた」を意味しない。早期利確・反転・清算・alt偏重で利益化できず<u>エッジがあっても赤字</u>の例が多い。よって挙動シグナルは<b>必ず実現損益と組み合わせて</b>評価する（分散通過18件中12件はmajors赤字の偽陽性だった）。</div>
</div>
...
<li><b>反復性</b>: 1回(N=1)でなく<b>別々の急変イベントで往復を繰り返す</b>か（下の第5節）。</li>
<li><b>協調</b>: 同じ<b>私的</b>資金元の仲間と<b>同一イベント・同時刻・同方向</b>。<br>
  ※ Gas.zip/Binance等の<b>公共サービス共有はノイズ</b>（誰でも使う）で協調の証拠にならない。</li>
</ul>
</div>
<div class="box bad">
<b>実例 — 「協調」に見えた cluster-A が崩れた話:</b> 当初、同一の Monad 資金元を共有する4ウォレットを協調網「cluster-A」と見立てた。
だが資金元 <code>Distributor [0xdf7479]</code> を調べると、<b class="r">単一日に約1万アドレスへ配った公開エアドロップ配布ハブ</b>だった。
＝「同じ資金元」は協調でなく<b>誰でも使う公共サービスの共有</b>。よって cluster-A は<b>解体・タグ撤去</b>し、メンバーは弱疑惑/除外へ降格した。
<b>これがまさに上の「公共サービス共有はノイズ」の生きた教訓</b>。確証ある私的協調は、現時点で<b class="r">ゼロ</b>。
</div>
...
<h2>6. 「遅効エッジ」 — 4時間では見えない先見性</h2>
<div class="box">
方向的中率を<b>{C.HIT_HORIZON_H}時間後の一点</b>だけで測ると、<u>数日かけて効くポジショニングをコイン投げと誤判定</u>する。そこで <b>1/4/12/24/48/72時間</b> の各地平線で測り直す。
<h3>① 何を見るか</h3>
<ul>
<li>各エントリの建玉後リターンを地平線ごとに算出し、さらに<b>その期間の市場平均リターン(ドリフト)を控除(トレンド補正)</b>する。＝「相場全体の上げ下げを超えて、自分の方向に動いたか」。</li>
<li><b>4hは≈0なのに24-72hで補正後リターンがプラス</b>＝『遅効エッジ』。動く数時間〜数日前に正しく仕込んでいた疑い。</li>
</ul>
<h3>② 偶然・トレンドを排除する分散テスト</h3>
1発の幸運や下落相場のショート便乗を除くため、補正後72h ≥ +3% に加えて <b>複数月(≥4)・複数銘柄(≥2)・両方向(ショート比0.15〜0.85)に分散</b>していることを要求する。
<div class="eg">結果: 台帳549件＋<b>未照会のリーダーボード層1500件</b>を精査し、分散テスト通過は計18件。</div>
<h3>③ 最重要の落とし穴: エッジ≠利益</h3>
<div class="box bad" style="margin:6px 0">
「建玉後72hに順方向へ動いた」は<b class="r">「儲けた」を意味しない</b>。早期利確・反転・清算・alt偏重で利益化できず、エッジがあっても赤字の例が多い。
実際、分散通過18件中<b>12件はmajors実現損益が赤字＝偽陽性</b>だった。よって挙動シグナルは<b>必ず実現損益と組み合わせて</b>評価する。
</div>
最終的に「遅効エッジ ＋ 分散 ＋ majors実現益が黒字(>$5万)」を満たした<b class="g">6件</b>を🟠弱い疑惑(監視継続)とした。
4h固定では全て除外に沈んでいた層で、<b>単一地平線の盲点を突いて初めて浮上</b>した。ただし「実力スイングトレーダー」と「遅効カタリストへの先行」は挙動だけでは断定不可で、最有力の監視対象に留める。
</div>
...
<tr><td>🔴 インサイダー疑惑</td><td>急変直前の先行建玉を<b>反復</b>（別々のイベントで往復≥3級）。単発(N=1)・公共サービス共有・低い方向的中率は不可。＝動く前に正しく賭けるのを繰り返す疑い</td></tr>
<tr><td>🟠 弱い疑惑</td><td>先行は出たが<b>反復が確認できない（N=1止まり）</b>等。監視継続だが断定しない中間層</td></tr>
