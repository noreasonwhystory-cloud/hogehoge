...
監視対象(プロ/MM/弱い疑惑)のポジション エントリー/クローズを HL WebSocket で即検知し Discord 通知。
さらに現在の建玉を定期ポーリングしてライブ ページ(:PORT)で確認できる。Nansen不要・HL公開APIのみ。

通知方針:
 - 本物/alt主体プロ・弱い疑惑 … WS userFills を購読し『Open*/Close*』を即通知(方向性のある層)
 - 高頻度MM … 約定が膨大ゆえ毎fill通知せず、ポーリングで大口ポジ変動(>MM_NOTIFY_MIN)のみ通知

環境変数:
 DISCORD_WEBHOOK  … Discord webhook URL(必須)
 WATCH_PATH       … watch_addresses.json のパス or raw URL (既定: data/watch_addresses.json)
 PORT             … ライブページのポート(既定 8080)
 POLL_SEC         … 建玉ポーリング間隔秒(既定 60)
 MM_NOTIFY_MIN    … MM大口ポジ変動の通知閾値USD(既定 500000)

使い方: DISCORD_WEBHOOK=... python hl_realtime.py
"""
import os
import json
import time
...
WATCH = {}        # addr -> {label, position, notify}
POSITIONS = {}    # addr -> {acct, pos:[...], t}
FEED = []         # 直近の通知イベント
PREV = {}         # addr -> {coin: signed_notional}  (MM差分用)
...
async def discord(session, title, desc, color):
    if not WEBHOOK:
        return
    try:
        await session.post(WEBHOOK, json={"embeds": [{"title": title[:240], "description": desc[:3500], "color": color}]},
...
async def emit_fill(session, a, f):
    ...
    isopen = dirv.startswith("Open")
    emoji = "🟢" if isopen else "🔴"
    color = 0x3fb950 if isopen else 0xff5d6c
    desc = (f"**{dirv} {coin}**  ${notion:,.0f}\nsz {sz} @ {px}"
            + (f"  決済PnL ${pnl:+,.0f}" if abs(pnl) > 1e-9 else "") + f"\n`{a}`")
    FEED.insert(0, {"t": int(f.get("time", 0)), "label": lbl, "pos": pos, "dir": dirv,
                    "coin": coin, "notion": round(notion), "pnl": round(pnl), "open": isopen})
    del FEED[300:]
    STATE["events"] += 1
    await discord(session, f"{emoji} {lbl} [{pos}]", desc, color)
        ...
        for a in list(WATCH.keys()):
            try:
                ...
                for ap in st.get("assetPositions", []):
                    ...
                    coin = p.get("coin")
                    ps.append({"coin": coin, "dir": "L" if szi > 0 else "S", "notional": round(notion),
                ...
                if WATCH.get(a, {}).get("position") == "高頻度MM":      # MMは大口ポジ変動のみ通知
                    prev = PREV.get(a, {})
                    for coin, sig in cur.items():
                        if abs(sig - prev.get(coin, 0)) >= MM_NOTIFY_MIN:
                            lbl = WATCH[a].get("label", a[:10])
                            await discord(session, f"🟣 {lbl} [MM大口]",
                                          f"{coin} 建玉変動 ${abs(sig-prev.get(coin,0)):,.0f}（現在 ${abs(sig):,.0f}）\n`{a}`", 0xa78bfa)
                    PREV[a] = cur
            except Exception:
                pass
...
async def handle_index(request):
    ...
    for a, d in held:
        ...
        pos_html = " ".join(
            f"<span class='p {'l' if p['dir']=='L' else 's'}'>{esc(p['coin'])}{p['dir']} ${p['notional']:,}"
            f"(<span class={'g' if p['upnl']>=0 else 'r'}>{p['upnl']:+,}</span>)</span>" for p in d["pos"])
        rows += (f"<tr><td><b style='color:{col}'>{esc(w['position'])}</b></td>"
                 f"<td>{esc(w['label'])}<br><code>{esc(a[:14])}…</code></td>"
                 f"<td>${d['acct']:,}</td><td>{pos_html}</td></tr>")
    feed = ""
    for e in FEED[:60]:
        em = "🟢" if e["open"] else "🔴"
        t = time.strftime("%m-%d %H:%M", time.gmtime(e["t"] / 1000)) if e["t"] else ""
        feed += (f"<tr><td>{t}</td><td>{em} {esc(e['label'])}</td><td>{esc(e['dir'])} {esc(e['coin'])}</td>"
                 f"<td>${e['notion']:,}</td><td>{('$'+format(e['pnl'],'+,')) if e['pnl'] else ''}</td></tr>")
...
通知イベント{STATE['events']} ／ 最終ポーリング {time.strftime('%H:%M:%S',time.gmtime(STATE['last_poll'])) if STATE['last_poll'] else '-'}UTC ／ 30秒自動更新</div>
<h2>🔔 最近のエントリー/クローズ（直近60）</h2>
<table><tr><th>時刻</th><th>ウォレット</th><th>アクション</th><th>規模</th><th>決済PnL</th></tr>{feed or '<tr><td colspan=5>まだイベントなし(WS購読中)</td></tr>'}</table>
<h2>📊 現在の建玉（{len(held)}件が保有中）</h2>
...
async def main():
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        ...
        await discord(session, "🚀 HLリアルタイム監視 起動",
                      f"監視{len(WATCH)}件 / 通知対象{sum(1 for w in WATCH.values() if w.get('notify'))}件 / ポート{PORT}", 0x4ea1ff)
