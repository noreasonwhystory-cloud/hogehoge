...
def main():
    ...
    behav = [
        ...
        ("⑪ 複数地平線の方向的中（1〜72h）★突破口",
         ...
         "<b class='g'>4h固定が見落としていた遅効エッジを発見</b>。台帳549＋未照会1500件から、majors実現益も黒字の<b>6件</b>を監視へ。<br><b class='r'>教訓:「建玉後72hに順方向」≠「儲けた」</b>(分散通過18件中12件はmajors赤字の偽陽性)＝シグナルは実現損益と必ず組合せ。"),
...
ただし4h固定で見落としていた<b>「遅効エッジ」黒字6件</b>と、出金集中の<b>hit-and-run候補4件</b>を最有力の監視対象として残す。
